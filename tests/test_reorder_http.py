...
def _crear_sucursal(admin, nombre):
    resp = admin.get("/sucursales/nuevo")
    token = extraer_csrf(resp.data)
    admin.post("/sucursales/nuevo", data={"csrf_token": token, "name": nombre}, follow_redirects=True)
...
def test_reordenar_por_http_aplica_el_nuevo_orden(admin):
    ...
    resp = admin.get("/sucursales/")
    csrf_meta = extraer_csrf_meta(resp.data)

    resp = admin.post(
        "/sucursales/reordenar",
